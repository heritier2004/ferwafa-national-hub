...
function initApp() {
    ...
    if (!userLogo || userLogo === "null" || userLogo === "undefined" || userLogo.trim() === "") {
        userLogo = null;
    }
    ...
    brandingContainers.forEach(container => {
        if (userLogo) {
            container.innerHTML = `<img src="${userLogo}" style="width:100%; height:100%; object-fit:contain; border-radius:inherit;">`;
            container.style.overflow = 'hidden';
            container.style.background = 'transparent';
        } else {
            // Keep shield/fallback if already present, otherwise add a clean Lucide shield placeholder
            if (!container.querySelector('i') && !container.querySelector('svg')) {
                container.innerHTML = `<i data-lucide="shield" style="width:20px; height:20px; color:var(--text-secondary);"></i>`;
            }
        }
    });
...
// 🛡️ FRONTEND SAFETY LAYER: Global Image Error Handler
window.addEventListener('error', function(e) {
    ...
    if (target.tagName && target.tagName.toLowerCase() === 'img') {
        ...
        target.style.objectFit = "contain"; // Ensure fallback fits
        target.style.background = "#1E293B"; // Dark surface background for contrast
        target.style.padding = "10px";
        target.style.border = "1px solid rgba(255,255,255,0.05)";
        target.style.borderRadius = "inherit"; // Inherit border radius
        
        // Determine context to provide appropriate fallback
